...
func TestExtractPaths(t *testing.T) {
	tests := []struct {
		...
		{"env var is not a path", "Write to $OUT/out.md or $HOME/$X/y.md.", nil},
		{"home var is a path", "Write to $HOME/out.md and \"${HOME}/logs/x.log\".", []string{"$HOME/out.md", "$HOME/logs/x.log"}},
		{"shebang interpreter", "#!/opt/homebrew/bin/python3\nprint(1)", []string{"/opt/homebrew/bin/python3"}},
		{"shell redirects", "cd /srv/app && make >out/build.log 2>/dev/null", []string{"/srv/app", "out/build.log", "/dev/null"}},
