...
| `name` | display name |
| `kind` | `agent` (default, and what an absent value means) \| `script` — a script job runs `prompt` as a program, with no provider, model or allowance |
| `prompt` | the instruction sent to Claude, or the script itself for `kind = "script"` (FA-01) |
...
| 17 | **Provider core** — Claude execution behind an adapter, with a registry, capabilities and normalized results | ✅ done | First stage of the multi-provider work (the multi-provider refinement, "PR 1"). New `internal/provider` package: a **provider instance** (stable id, adapter kind, name, binary path, configuration directory, default model, enabled) is what a task runs on; a **registry** maps an adapter *kind* to its implementation; **capabilities** answer what a harness can do so no caller has to branch on a provider name; **access modes** (`provider-default`, `read-only`, `workspace-write`, `full-access`) express authority provider-neutrally and an adapter refuses a mode it cannot enforce; a **collector** folds normalized events (`session_started`, `completed`, `rate_limited`, `authentication_failed`, `model_rejected`, `failed`) into one `Result`. All Claude specifics moved into `internal/provider/claudecode`: CLI flags, binary detection (now cached after a successful probe), `CLAUDE_CONFIG_DIR`, and the stream-json parser. The executor and engine are provider-neutral; the engine resolves provider+model through the documented inheritance table and never substitutes another provider when a task names one it cannot honour (phase 18 changed the outcome of that from a failed run to a blocked, still-queued task). Migration is a pure mapping of the existing settings (`claude_path`, `default_model`) onto the `claude` instance of kind `claude-code`, so a fresh install has the provider too and nothing on disk changes. Golden argument tests pin the invocation byte for byte; a fake third adapter proves a new harness needs no engine or scheduler change. No user-visible behaviour changed. **Still to come:** provider persistence, health/readiness and the provider CLI/API (PR 2), the Codex adapter, full provider management in Settings and the Beta opt-in (PR 3), provider-aware scheduling and the secondary flows (PR 4), durable multi-provider workflows (PR 5), and the SwarmQ rename (PR 6). |
| 18 | **Provider configuration & health** — providers on disk, readiness checks, provider API/CLI, Settings cards and blocked tasks | ✅ done | Second stage of the multi-provider work (the multi-provider refinement, "PR 2"). Provider instances are **persisted** in `config.toml` as `[[providers]]`, and the global `default_provider` says which one a task runs on when it names none. **Migration** seeds the `claude` instance from the retired `claude_path` and `default_model` settings and then drops those keys, so the file has one answer to "which claude, which model?"; it is idempotent, and a fresh install has the instance too. **Three check levels** are kept apart: configuration validity (id charset, unknown kind, relative path) rejects the change; **readiness** (`ready`, `not_installed`, `not_authenticated`, `invalid_configuration`, `disabled`, `check_failed`) keeps the provider visible but not runnable; runtime problems stay on the run. The Claude Code check spends no model usage — a filesystem look, `--version`, and `claude auth status --json`, whose *output* decides (the CLI prints a complete document and still exits non-zero when logged out) and of which only `loggedIn` is read, so the account's email and organisation never reach a log or the API. Verdicts are cached for 30s and keyed by the instance's own settings, so editing a path invalidates them by itself; the daemon shares one checker with the API, and re-probes without the cache immediately before a start. **Blocked tasks** stay queued: no run is recorded, no concurrency slot is taken, one-shot and cron state do not advance, the Queue row shows a red *blocked* badge with the reason, and the task starts by itself when the provider recovers. A health transition notifies once — the memo lives in `state.json`, so a restart does not repeat it. New tasks naming an unready provider are refused (app and CLI) with a concrete reason; `run-now` reports it to the caller. New `claudeq provider list|show|check|add|edit|enable|disable|default|rm`, `--provider` on `add`/`edit`/`queue` (and in the `$EDITOR` document), `claudeq settings --default-provider`, and `/api/providers` endpoints. Settings gains a **Providers** tab whose card carries status, binary, configuration directory and default model, replacing the General *Default model* row and the System *Claude Code CLI* group; both path fields take a leading `~` and store it resolved. Prompt review, the feedback chat and *Continue with Claude* resolve their binary and model from the provider instance instead of the old global settings. **Still to come:** provider-aware scheduling and the secondary flows (PR 4), durable multi-provider workflows (PR 5), and the SwarmQ rename (PR 6). |
| 19 | **Codex & provider management in the app** — a second harness, and the whole provider surface in Settings | ✅ done | Third stage of the multi-provider work ("PR 3"). New `internal/provider/codex` adapter, built on what the spike measured against `codex-cli 0.154.0` and tested against the sanitized streams it captured — no test starts a Codex process. `codex exec --json` with the prompt piped to **stdin** and `-` as the argument (passing it as an argument while stdin is not a terminal made the CLI announce it was reading stdin too); the JSONL stream folds into the same normalized events every adapter reports, classifying a failed turn as auth, rate limit, rejected model or plain failure from the *last* problem reported. Readiness is the binary, `--version` and `login status` — never `codex exec`, which without credentials retries both transports five times each. `CODEX_HOME` per instance; `developer_instructions` **replaces** the configured value, so the instance's own instructions are read and carried along rather than dropped, which is the conflict the spike said had to be resolved explicitly. Codex's sandbox flag means it can enforce read-only and workspace-write, which Claude Code cannot. The adapter contract grew what a second harness needs: a prompt on stdin, a reasoning effort (`task.reasoning_effort`, `--reasoning-effort`), model suggestions **per provider** (moved off the API onto the adapter), and the command that reopens a session in a terminal — so *Continue with Claude* reopens a Codex thread with Codex. Cancellation now tears down the whole descendant tree: the spike measured that Codex starts tool processes in their own process groups, so killing the harness's group left `/bin/sleep` running under PID 1. **Settings → Providers** became full management — each provider is its own settings block headed by its name and type — written by the page's single Save, with an **Add provider** sheet below them — with the store's rules, so the app refuses exactly what the CLI refuses; the task form gained a Provider field (only ready instances offered) and a reasoning-effort field that appears where the harness takes one. A **Beta features** switch (Settings → System, `beta_features`; the old `show_codex_beta` migrates) gates the two unfinished parts — adding providers at all, and Codex — and the Add-provider button is marked as beta where it appears; it is presentation state only — the adapter is always registered, the API and CLI always accept Codex, the scheduler never reads the flag, and a CLI-created Codex task runs and stays visible while the controls are hidden. Nothing in the app or API branches on a provider's name: what to offer, label or show is answered by adapter capabilities. The **rate-limit gate is now per provider instance** (`limit.Gates`) — brought forward from PR 4 because the moment a second harness exists, a Codex 429 pausing Claude Code is a defect, not a deferral; an allowance belongs to one account, so only that provider's tasks wait. The scheduler also stopped probing on idle ticks: it asks a harness only about tasks that are actually due, which is the moment right before a start rather than every five seconds, and it reads the scheduling state *after* the probes so a run that finished meanwhile is not evaluated against a stale snapshot. **Still to come:** provider-aware scheduling and the secondary flows (PR 4), durable multi-provider workflows (PR 5), and the SwarmQ rename (PR 6). |
| 20 | **Provider-aware scheduling & secondary flows** — everything around a run that still assumed one harness | ✅ done | Fourth stage of the multi-provider work ("PR 4"). A session belongs to the harness that issued it: a pending resume carries its provider, so a task moved between providers starts fresh and says so in the run's own log, and "Continue with Claude" reopens the session on the provider the *run* used, with the authority that run had. Prompt review and the feedback assistant became **asides** (`internal/aside`): a tool-free, project-free, optionally resumable question claudeq asks a harness on its own behalf, behind the adapter, with its own provider and model in Settings and the CLI. Claude Code claims the capability; Codex does not, because a tool-free neutral-directory schema-bound turn was never measured in the spike. Task bundles carry a provider *hint* (kind, model, exporter's name) instead of a local instance id; import takes the one matching provider or leaves the choice to the operator, and `claudeq import` takes `--provider`/`--model`. Usage splits its totals per provider and Activity names the harness a run used. Self-queue inherits the account the parent actually ran on. Feedback became a page of its own instead of a modal. |
| 21 | **Durable multi-provider workflows** — fan-out to several providers and a join that consolidates them | ✅ done | Fifth stage of the multi-provider work ("PR 5"). A task can name the one-shot jobs it waits for; it is not due until every one has a terminal result. A job pausing on a rate limit has *not* finished (its session is scheduled to continue), while a failed, auth-failed or canceled one has — an unattended digest that never appears is worse than one that names its missing input — and so has a job that has vanished. Dependencies are fixed at queue time and may only name jobs that already exist, which rules out a cycle without a cycle check; a recurring job can neither depend nor be depended on. The harness's final answer is kept on the run record, bounded, and `--include-results` puts the dependencies' answers in front of the join's prompt — fenced, labelled as data, budgeted, with truncation stated and the full log path named. Runs carry a workflow id (a job queued by a run joins its parent's; a run with none starts one under its own run id), which also groups ordinary self-queued chains. `queue --json` returns the new job id so a fan-out can feed it into `--depends-on`. The built-in system prompt teaches the shape rather than claudeq parsing the operator's prose. Queue says what a waiting job is waiting for; Activity groups a workflow's runs into one block. Verified end to end with a Morning Digest across three fake providers including a failed child, and across a daemon restart. |
| 22 | **Rate-limit fallback** — a provider names who takes its tasks while its allowance is used up | ✅ done | A provider entry carries `fallback_provider`: when its own limit gate is shut, the scheduler resolves its due tasks onto that instance instead of holding them, following the chain to the first account that has allowance left and can actually run (a hop that is blocked, switched off or unready is stepped over, a cycle ends the walk, and a chain with nothing usable in it waits as before). The handing-over provider may also name the model the substitute runs (`fallback_model`, cleared with the fallback itself); left open, the task's model travels only between instances of the same kind. It answers the rate limit only — a missing, logged-out or disabled provider is still never substituted. The interrupted session stays with the provider that issued it, so the substitute starts fresh and the run's log names both. Configurable on the provider's block in Settings, over `PUT /api/providers/{id}` and with `claudeq provider edit --fallback`; `config.toml` refuses a fallback that names nothing, itself, or a cycle, and removing a provider somebody falls back to is refused by name. The waiting banner names the substitute and says the queue keeps running. |
| 23 | **Queue groups** — tasks filed into foldable groups by drag and drop | ✅ done | A task carries a free-text `group` (config.toml, `--group` on add/edit/queue, the editor document). The stored order stays the rendered one: `app.GroupedOrder` keeps the tasks of a group together without reordering them among themselves, and the move endpoint takes an optional `group` so a drop writes position and group in one request (`POST /api/tasks/{id}/move?to=N&group=NAME`, empty = ungrouped). A group exists only as long as a task names it — nothing to create or delete. Dragging a header moves a whole section (`POST /api/groups/move`, `before` naming the section to sit in front of, absent = the end), and `POST /api/groups/rename` renames one — onto an existing name it merges the two sections, which the dashboard confirms first. Whether a section is folded lives in `state.json` (`collapsed_groups`, `GET /api/groups`, `POST /api/groups/collapse`) and is pruned with the last task of a group. In the queue each row now says which provider and model it runs on, where its id used to stand. |
| 24 | **Agent and script jobs** — a second job kind that runs deterministically, without a model | ✅ done | A task carries a `kind`: `agent` (the default, and what every task written before this says by saying nothing) or `script`, where the prompt is the program to run. A script job is written to a temporary file and executed — a shebang picks the interpreter, otherwise `/bin/sh` — in the task's working directory, with the same environment an agent run gets (`CLAUDEQ_BIN`, `CLAUDEQ_HOME`, `CLAUDEQ_RUN_ID`, `CLAUDEQ_TASK_ID`, `CLAUDEQ_WORKFLOW_ID`, `CLAUDEQ_PARENT_TASK`), so it can queue agent work and publish or notify like anything else. The exit code is the whole classification (`0` success, else failure): there is no stream to parse and nothing is inferred from the output. Both streams reach the run log; stdout alone is kept as the answer, tail-first, because a script says what matters last. Everything around the run is shared with an agent job — triggers, priority, the exclusive/parallel slot, groups, quiet history, the idle watchdog, process-group cancellation, dependencies (`--include-results` reaches a script on **stdin**, since a program cannot have a digest prepended to it). What is skipped is everything provider-shaped: no resolution, no readiness probe, no session, and no rate-limit gate — an allowance belongs to an account and a script job has none, so a watcher keeps ticking while every provider waits out its limit and files the work for when it reopens. A script job may not name a provider, model, reasoning effort or the permission bypass; validation refuses them rather than storing settings that do nothing, and switching an existing task to `script` clears them. Its run records `Script` where the provider goes, and is left out of the per-provider usage split. `claudeq queue` never inherits the kind — a script's follow-up is the agent work it found — unless `--kind script` asks for it. Surfaces: a **Job type** switch in the task form (which hides the provider, model and permission fields and switches the prompt review to a script's rules: missing paths incl. the shebang interpreter and `$HOME/…`, writes into missing directories, anything interactive, and — for a shell script — each command looked up on the daemon's own PATH, with its install location elsewhere when it is missing there, so the rewrite can call it by absolute path), a `script` badge in the queue, `--kind` on `add`/`edit`/`queue`, a `KIND` column in `claudeq list`, and `kind` in the `$EDITOR` document and in `.claudeq` bundles (which carry no provider hint for a script).
...
- **Claude binary resolution** — the launchd daemon runs with a minimal `PATH`
  ...
  auto-detects the binary (`claudecode.DetectBinary`: `CLAUDEQ_CLAUDE_BIN` →
