...
| 5 | **App (GUI)** — task management, news dashboard, history, logs, settings | ✅ done (web) | Local 127.0.0.1 HTTP API + embedded web dashboard served by `claudeqd run` (`--addr`). Covers task CRUD/reorder/run-now, news+unread, history, log view, settings. Verified end-to-end in a real browser (render, nav, log fetch, mark-all-read) + API tests. **Native Wails window wrap still deferred** — flagged for user review (deviation from D2). |
