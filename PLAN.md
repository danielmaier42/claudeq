...
| 5b | **Native window (Wails)** — wrap the dashboard in a WKWebView app window | ⏳ not started | Thin native window over the daemon (D2/D3). Adds a real app window + a native accent bridge (read/observe `AppleAccentColor` in the app's GUI-session context, inject `--accent`) — the one thing WebKit's CSS `AccentColor` won't give reliably. **GUI can't be run/screenshotted headlessly here** → build is compile-verified; window/accent verified by the user. |
