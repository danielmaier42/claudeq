...
| 11 | **Task notifications & quiet history** — a run sends its own notification; frequent jobs stay out of history | ✅ done | `claudeq notify --title T --message M [--url U]` lets a run (a watcher job, typically) alert the operator without publishing a throwaway artifact. Mirrors the artifact mechanism: the CLI drops the message into an outbox (`notifications.json`, atomic write under the shared cross-process `flock`), attributed via `CLAUDEQ_TASK_ID`/`CLAUDEQ_PARENT_TASK`/`CLAUDEQ_RUN_ID`; the daemon takes the whole outbox atomically on its next tick (empty outbox = plain read, no lock) and sends over its existing channels — the task name is appended to the body, `--url` (absolute http/https only) travels as Pushover's supplementary `url` and as `cq_url` in the macOS notification's userInfo, where the window app opens it with `NSWorkspace` on click instead of raising the window. A third built-in system-prompt block documents the command and tells Claude to use it only when the finding warrants it (the run outcome is announced separately). **Quiet history** (`quiet_history` on the task; `--quiet-history` on add/edit, a switch in the task form, inherited by `claudeq queue`): a run that ends in `success` is removed from history with its log (`Store.DropRun`, a compaction rewrite sharing the reader/writer with `PruneHistory`) and its read-status forgotten; failed/auth/rate-limited/canceled runs are kept. Runs of quiet tasks are never counted as unread (API + `claudeq status`), so a 15-minute watcher neither floods Activity nor evicts real runs from the 500-run history. Unit tests: outbox queue/take/corrupt-file, DropRun, engine delivery-once + attribution + no-notifier, quiet success dropped / failure kept / visible while running, Pushover `url` form field, CLI validation (required flags, URL scheme), patch/TOML/API round-trips. |
